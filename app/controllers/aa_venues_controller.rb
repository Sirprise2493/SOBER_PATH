class AaVenuesController < ApplicationController
  ...
  def edit
    ...
    render partial: "pages/venue_form", locals: { venue: @venue }
  end
  ...
  def update
    if @venue.update(venue_params)
      ...
      @venue.aa_meetings.build if @venue.aa_meetings.empty?
      render partial: "pages/venue_form", locals: { venue: @venue }, status: :unprocessable_entity
    end
  end
  ...
  def venue_params
    params.require(:aa_venue).permit(
      :name, :street, :zip, :city, :country, :website_url, :notes,
      :latitude, :longitude,
